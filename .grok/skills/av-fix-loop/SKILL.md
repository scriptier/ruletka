...
- Ship pending web hop **3 / 3c / 6–8** (UI deploy + hard-refresh) **before** more pure-wait budget cuts.  
- **GOAL_MET** only with `product.ok` after deploy; journal mto/mta alone is **insufficient**.  
- Prefer **PC paint-first / frames-gated media-OK** over ICE thrash when the complaint is “PC feels slow” (chrome/paint lag ≠ more force_relay).  
- Wiki: `knowledge/wiki/linking-speed.md`. Spec: `knowledge/specs/current-linking-speed.md`.
